...
> **2026-07-12 업데이트**: 출시 형태가 바뀌었어 — 앱이 제공하는 연동 소스는 이제
> **애플 건강 / 헬스 커넥트 두 허브뿐**이야. NRC·Strava·Garmin은 별도 소스가 아니라
> "브랜드 앱 → 허브에 저장 → '기기에서 기록 가져오기'" 경로로만 들어오고, MyNB는
> 완전히 제거됐어(기록을 생산하지 않는 앱). 아래 브랜드별 시나리오는 그 허브 경유
> 흐름을 검증하는 용도로 읽으면 되고, "직접 소스로 연결"이라는 표현은 폐기됐어.
> 가져오기는 항상 수동 버튼이야(자동 동기화 없음).
