...
//
// 시즌 S = [YYYY-MM-01 0시 KST, 다음 달 1일 0시 KST). 끝나고 48시간 뒤 봉인한다.
...
//
// 기여 인정: isCompetitiveRun(앱 GPS·매치 기록, 차량 판정 제외) + isRunCountable(endedAt이
// 멤버십 창 안, 서버 createdAt이 창+48h 안) → 같은 사람의 시간이 겹친 기록은 가장 긴 것 하나 →
// endedAt의 KST 날짜(run.date 아님)로 하루 45km 상한 → roundDistanceKm 고정 순서 누적.
...
//
// 직전 시즌이 끝났는데 아직 봉인 전(시즌 첫 48시간)이면 봉인이 할 계산을 지금 그대로 한다
// (적대 리뷰 2026-09-18): 원장이 없다고 30을 쓰면 3일 0시 봉인 순간 P가 30 → avgKm로 뛰어
// 아무도 안 뛰었는데 모든 점수가 바뀌고, 크기가 다른 크루끼리 순위가 밤사이 뒤집힌다. 끝난
// 시즌의 순위는 늦게 올라온 기록(창+48h) 말고는 변하지 않으니 이 값은 봉인 값으로 이어진다.
...
// 멤버 기여 = Σ_KST하루 min(45, 그날 기록 합). 순서 고정: endedAt 오름차순(같으면 id).
function sumCappedContributionKm(runs) {
  const ordered = [...runs].sort((left, right) => left.endMs - right.endMs
    || String(left.id).localeCompare(String(right.id)));
  const dayKmByKey = new Map();

  for (const run of ordered) {
    const dayKey = kstDateKeyOfMs(run.endMs);
    dayKmByKey.set(dayKey, roundDistanceKm((dayKmByKey.get(dayKey) ?? 0) + run.km));
  }

  let totalKm = 0;
  for (const dayKm of dayKmByKey.values()) {
    totalKm = roundDistanceKm(totalKm + Math.min(CREW_DAY_CAP_KM, dayKm));
  }
  return totalKm;
}
...
export function buildCrewSeasonStandings(store, seasonKey, nowMs = Date.now()) {
  ...
  for (const row of store.crewMembers ?? []) {
    ...
    const countsFromMs = parseMs(row.countsFrom, Number.POSITIVE_INFINITY);
  ...
  // store.runs 한 번 훑기 — 창이 있는 사람의 경쟁 기록만 후보로.
  const candidatesByUserId = new Map();
  for (const run of store.runs ?? []) {
    const windows = windowsByUserId.get(run?.userId);
    if (!windows || !isCompetitiveRun(run)) {
      continue;
    }

    const window = windows.find((entry) => isRunCountable(run, entry.fromMs, entry.toMs));
    if (!window) {
      continue;
    }
  ...
  for (const candidates of candidatesByUserId.values()) {
    for (const kept of dedupeOverlappingRuns(candidates)) {
  ...
  for (const group of orderedGroups) {
    group.contributionKm = sumCappedContributionKm(group.runs);

    // 시즌 멤버 판정 (7일 규칙). 프리시즌은 규칙을 끈다 — 인정이 시작된 사람은 모두 멤버.
    let isSeasonMember;
    if (isPreseason) {
...
// 봉인 대상: 유예(48h)까지 지난 완료 시즌 — (now − 유예)가 속한 KST 달 이전의 모든 시즌.
...
function notifyCrewSeasonResult(store, award, rankedRows, now) {
  ...
  for (const row of rankedRows) {
    const body = championIds.has(row.crewId)
      ? `${monthLabel} 크루대전 우승: ${row.name} ★`
      : `${monthLabel} 크루대전 결과: ${row.name} ${row.rank}위 / ${award.rankedCount}크루`;

    for (const userId of row.memberUserIds) {
      appendUserNotification(store, {
        userId,
        type: 'crew_season_result',
        title: '크루대전 결과',
        body,
        data: { seasonKey: award.seasonKey, crewId: row.crewId },
        nowIso: () => nowIso,
      });
    }
  }
...
// 봉인 스윕 — 유예가 지난 미봉인 시즌을 오래된 순서로 봉인한다. 멱등: seasonKey 유일성을
// 잠금(mutateStore) 안에서 다시 확인한다. 순서가 중요하다: 다음 시즌의 고정 P가 방금 봉인한
// 직전 시즌 avgKm에서 나온다(서버가 두 달 멈췄다 켜져도 같은 값).
export function sweepCrewSeasons(store, now = new Date()) {
  ...
  for (const seasonKey of resolveSealableCrewSeasonKeys(now)) {
    ...
    if (!award.isPreseason) {
      notifyCrewSeasonResult(store, award, rankedRows, now);
    }
