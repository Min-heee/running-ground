...
export function createJsonLeagueRepository({
  ...
  return {
    ...
    //
    // 토큰을 받지 않는 유일한 읽기 경로다. 로그인 경로와 두 가지가 다르다: '내 별' 정보가
    // 없고, 은하 안의 러너는 **상위 60명까지만** 개별로 내려간다(나머지는 익명 집계 —
    // universeBuilder.PUBLIC_PLANET_RENDER_CAP). 로그인한 앱은 인당 1행성 규칙(오너
    // 2026-08-21)대로 전원을 받지만, 공개 웹은 지역 id ~230개만 돌면 명부를 통째로 긁어갈
    // 수 있어 경계를 남겼다.
    //
    // 주의: 이 경로가 열리는 순간 (은하당 상위 60명의) 회원 이름과 거리가 **공개 인터넷에
    // 노출**된다. 앱 안에서는 로그인한 회원끼리만 보이던 정보이고, 개별 비공개(옵트아웃)는
    // 아직 없다. 게이트를 켜기 전에 이 범위를 오너가 다시 확인해야 한다.
