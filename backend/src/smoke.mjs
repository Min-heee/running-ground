...
async function main() {
  ...
  try {
    ...
    // 지역 보드의 거리(오너 2026-07-31)는 전체 주간 거리 — 가져온/수동 기록도 포함해
    // 홈 카드의 개인 주간 거리와 같은 값이어야 한다.
    assert(districtPersonal.weeklyDistanceKm === expectedMetrics.currentWeekDistanceKm, '구 내 개인 주간 거리가 전체 주간 거리와 맞지 않아요.');
    ...
    const trackedRun = await request('/runs/tracked', {
      ...
      body: JSON.stringify({
        ...
        cadenceSpm: 176,
        elevationGainM: 42,
        startedAt: `${formatDate(addDays(today, -1))}T06:30:00.000Z`,
        endedAt: `${formatDate(addDays(today, -1))}T06:57:55.000Z`,
        route: [
          { latitude: 37.5665, longitude: 126.978, altitude: 38.1, timestamp: `${formatDate(addDays(today, -1))}T06:30:00.000Z` },
          { latitude: 37.5671, longitude: 126.9792, altitude: 40.8, timestamp: `${formatDate(addDays(today, -1))}T06:38:00.000Z` },
          { latitude: 37.5684, longitude: 126.9811, altitude: 44.2, timestamp: `${formatDate(addDays(today, -1))}T06:57:55.000Z` },
        ],
      }),
      expectedStatuses: [201],
    });
