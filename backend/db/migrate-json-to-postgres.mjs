...
function buildRunRows(store, issues, knownUserIds) {
  ...
  for (const run of asArray(store.runs)) {
    ...
    rows.push({
      ...
      cadence_audit: run.cadenceAudit && typeof run.cadenceAudit === 'object' ? sqlJson(run.cadenceAudit) : 'null',
  ...
  const tables = [
    ...
    },
    {
      ...
      columns: [
        ...
        'route', 'duration_seconds', 'cadence_spm', 'cadence_audit', 'elevation_gain_m', 'started_at', 'ended_at',
