...
2. `schema.sql`을 적용한다.
3. `npm --prefix backend run db:check`로 연결 상태를 확인한다.
4. JSON store를 읽어서 PostgreSQL에 넣는 migration script를 만든다.
5. smoke test를 JSON과 PostgreSQL 양쪽에서 통과시킨다.
6. TestFlight preview API부터 PostgreSQL로 전환한다.
