// Health access plugin: Android Health Connect + iOS Apple Health (HealthKit).
//
// The iOS half (entitlement, health usage-description Info.plist keys, the
// RunnigappAppleHealth native reader module, and the HealthKit framework link)
// was removed for the App Store 2.5.1 resolution (build 48 shipped
// HealthKit-free) and is restored here verbatim from git history for the
// post-approval build 49+. The JS side gates every Apple-Health surface on the
// RUNTIME PRESENCE of the RunnigappAppleHealth module (see
// src/integrations/appleHealthAvailability.ts), so the same OTA'd bundle keeps
// Apple Health hidden on build 48 and lights it up on binaries built with this
// plugin.
const {
  AndroidConfig,
  createRunOncePlugin,
