...
export function LiveMatchRaceBoardPage({
  matchMode,
  ...
  if (matchMode === 'duel' && roomLinkedDuelPlaceholderParticipants.length === 2) {
    ...
    const duelRows = sortRaceRows(roomLinkedDuelPlaceholderParticipants.map((participant) => ({
      id: participant.id,
  ...
  if (matchMode === 'group' && roomLinkedGroupPlaceholderParticipants.length > 0) {
    ...
    const rows = sortRaceRows(roomLinkedGroupPlaceholderParticipants.map((participant) => ({
      id: participant.id,
      name: participant.name,
      distanceKm: participant.distanceKm,
      remainingKm: Math.max(0, placeholderDistanceKm - participant.distanceKm),
