...
test('journey model: primary not connected → connect-first copy, no step complete', () => {
  ...
  assert.equal(model.headline, 'Android에서는 헬스 커넥트부터 연결하면 돼요.');
  assert.equal(model.body, '기본 연동 소스를 먼저 붙여두면 이후 기기 기록 가져오기, 홈 요약까지 한 흐름으로 연결돼요.');
  assert.deepEqual(model.steps.map((step) => [step.id, step.complete]), [
    ['primary', false],
    ['import', false],
    ['manual', false],
  ]);
  assert.equal(model.steps[0].description, 'Android에서 가장 먼저 연결할 기본 소스예요.');
  assert.equal(model.steps[1].description, '기본 소스를 연결하면 그다음 단계로 넘어갈 수 있어요.');
...
test('journey model: module present (build 49+) → iOS resolves Apple 건강 and offers import', () => {
  ...
  assert.equal(model.headline, 'Apple 건강는 준비됐고, 이제 기기 기록을 가져오면 돼요.');
});
...
test('journey model: primary connected + device readable → import copy and import button', () => {
  ...
  assert.equal(model.headline, '헬스 커넥트는 준비됐고, 이제 기기 기록을 가져오면 돼요.');
  assert.equal(model.body, "'기기에서 기록 가져오기' 버튼을 누르면 기기에 쌓인 러닝 기록을 바로 가져올 수 있어요.");
  assert.deepEqual(model.steps.map((step) => [step.id, step.complete]), [
    ['primary', true],
    ['import', false],
    ['manual', false],
  ]);
  assert.equal(model.steps[1].description, "'기기에서 기록 가져오기' 버튼을 누르면 기기에 쌓인 러닝 기록이 들어와요.");
  assert.equal(model.showImportButton, true);
});
...
test('journey model: primary connected, device not readable, manual open → manual-safety copy', () => {
  ...
  assert.equal(model.headline, '연동으로 안 들어온 기록은 수동 기록으로 바로 채울 수 있어요.');
  assert.equal(model.body, '가져오기로 안 들어온 날도 직접 입력만 하면 기록이 바로 반영돼요.');
  assert.deepEqual(model.steps.map((step) => [step.id, step.complete]), [
    ['primary', true],
    ['import', true],
    ['manual', true],
...
test('journey model: primary connected, device not readable, manual closed → open-safety-net copy', () => {
  ...
  assert.equal(model.headline, '소스 연결 다음엔 수동 입력 안전망까지 열어두면 든든해요.');
  assert.equal(model.body, '가져오기와 별개로 수동 입력 경로를 열어 두면 기록이 빌 일이 없어요.');
  assert.deepEqual(model.steps.map((step) => [step.id, step.complete]), [
    ['primary', true],
    ['import', false],
    ['manual', false],
  ...
  assert.equal(model.steps[1].description, "연동 관리에서 '기기에서 기록 가져오기' 버튼을 누르면 기록이 들어와요.");
});
