...
// dragging in the whole run-tracking / 연동관리 flow. On iOS the Apple-Health half is gated on
// the RUNTIME presence of the RunnigappAppleHealth module (see nativeHealth.ts /
// appleHealthAvailability.ts): the HealthKit-free build 48 reads null everywhere, build 49+
// lights up from the same OTA'd JS.
//
...
// import flow — we never block onboarding on the OS sheet itself. On an iOS binary without the
// RunnigappAppleHealth module (HealthKit-free build 48) getPreferredNativeHealthSource() is
// null, so this reads not-granted / not-askable there.
...
// module (Expo Go / HealthKit-free build 48 / unsupported) just returns false instead of crashing
// onboarding — on those binaries the preferred source is already null, so this no-ops to false
// without touching any health API.
